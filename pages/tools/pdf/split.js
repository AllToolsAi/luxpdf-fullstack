...
import { FiDownload, FiTrash2, FiFileText, FiInfo, FiPlus, FiX, FiCheck, FiArrowLeft } from 'react-icons/fi';
...
export default function SplitPDF() {
    ...
    const selectAllPages = () => {
        setSelectedPages(prev =>
            prev.length === pageImages.length
                ? []
                : pageImages.map(page => page.index)
        );
    };
    ...
    const splitPDF = async () => {
        ...
        try {
            ...
            const pagesToCopy = selectedPages.sort((a, b) => a - b).map(p => p - 1);
            ...
            a.href = url;
            a.download = `${file.name.replace(/\.pdf$/i, '')}_pages_${selectedPages.join('-')}.pdf`;
    ...
    return (
        <Layout>
            <SEO
                title="Split PDF | Extract Pages from PDF Documents"
                description="Free online tool to split PDF files. Select specific pages to extract and download them as a new PDF document."
                keywords="split pdf, extract pdf pages, pdf splitter, pdf tools, separate pdf"
            />

            <section className="py-16 min-h-[80vh]">
                <div className="max-w-screen-xl mx-auto px-4">
                    <main>
                        <h1 className="text-4xl font-bold text-center mb-4">Split PDF</h1>
                        ...
                        <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg max-w-2xl mx-auto space-y-6">
                            ...
                            {pageImages.length > 0 && (
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        >
                                            {selectedPages.length === pageImages.length ? 'Deselect all' : 'Select all'}
                                        </button>
                                    </div>
                                            ...
                                            >
                                                ...
                                                {selectedPages.includes(page.index) && (
                                                    <div className="absolute inset-0 bg-primary/20 flex items-center justify-center">
                                                        <FiCheck className="text-white bg-primary rounded-full p-1" />
                                                    </div>
                                                )}
                            ...
                            <div className="flex gap-4">
                                <button
                                    ...
                                    className={`flex-1 py-3 rounded-md text-lg font-semibold transition-colors ${
                                        !file || isProcessing || selectedPages.length === 0
                            ...
                            {success && !isProcessing && (
                                <div className="p-4 bg-green-50 rounded-lg">
                                    <p className="text-green-800 font-medium text-center">
                                        PDF split and downloaded successfully!
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="p-4 bg-blue-50 rounded-lg text-sm">
                                ...
                                <ul className="list-disc list-inside space-y-1 text-blue-700">
                                    <li>Upload a PDF file to select specific pages</li>
                                    <li>Click on page thumbnails to select/deselect</li>
                                    <li>Files are processed securely in your browser</li>
                                    <li>Supports files up to 50MB in size</li>
                                    <li>Download only the selected pages as a new PDF</li>
                                </ul>
                            </div>
